"""
Daily Carbon Calculator Service
...
class DailyCarbonCalculator:
    """Service for calculating daily carbon savings"""
        ...
        try:
            with open('logs/actual_carbon_intensity.csv', 'r') as f:
                ...
                for row in reader:
                    ...
                    if 'carbon_intensity_kgco2e_kwh' in row:
                        self.carbon_data_cache[timestamp] = float(row['carbon_intensity_kgco2e_kwh'])
                    else:
                        self.carbon_data_cache[timestamp] = float(row['carbon_intensity_kgco2_kwh'])
    ...
    async def calculate_user_daily_carbon(
        ...
        """Calculate carbon savings for a specific user on a specific date"""
    ...
    def _calculate_chore_carbon_saved(self, chore: Chore) -> float:
        ...
        return max(0, carbon_saved)  # Only count positive savings
    ...
    def _calculate_period_carbon_intensity(self, start_time: datetime, end_time: datetime) -> float:
        ...
        if relevant_intensities:
            ...
            return 0.500  # Default 500g CO2/kWh
    ...
    def _find_worst_continuous_period(self, target_date: date, duration_minutes: int) -> float:
        ...
        if not day_data:
            return 0.600  # Default worst case
    ...
    async def _calculate_month_to_date(
        ...
        """Calculate total carbon (CO2e) saved from month start up to a specific date"""
